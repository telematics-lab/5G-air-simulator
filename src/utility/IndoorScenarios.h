...
GetWalls(Femtocell* henb_cell, UserEquipment* ue)
{
  ...
  if(henb_building->GetBuildingType() == Building::TYPE_3GPP_5x5_grid)
    ...
    {
      //XXX: To BE REVISED
      /*int X_gNB_index = henb_cell_id%10;
      int Y_gNB_index = (int) floor( (double) gNodeBCellID / (double) 10 )%4;

      int X_ue_index = UserCellID%10;
      int Y_ue_index = (int) floor( (double) UserCellID / (double) 10 )%4;

      nbWalls[1] = abs( X_gNB_index - X_ue_index )
                              + abs ( Y_gNB_index - Y_ue_index );

      if ( ( Y_gNB_index<=1 && Y_ue_index>1 ) || ( Y_gNB_index>1 && Y_ue_index<=1 ) )
      {
        nbWalls[0]  = 2;
        nbWalls[1] -= 1;
      }*/
