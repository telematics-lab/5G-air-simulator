...
static void SingleCellWithoutInterferenceUplink (int argc, char *argv[])
{
    ...
    //the next four lines are necessary for the uplink with RACH
    ...
    //Create ENodeB
    GNodeB* enb = new GNodeB (1, cell, 0, 0);
    ...
    //the next line is for uplink scheduling
    enb->SetULScheduler(GNodeB::ULScheduler_TYPE_MAXIMUM_THROUGHPUT);
    ...
    for (int i = 0; i < nbUE; i++)
    {
        ...
        for (int j = 0; j < nbVoIP; j++)
        {
            ...
            VoIPApplication[voipApplication].SetDestination (gw);
        ...
        for (int j = 0; j < nbVideo; j++)
        {
            ...
            VideoApplication[videoApplication].SetDestination (gw);
        ...
        for (int j = 0; j < nbBE; j++)
        {
            ...
            BEApplication[beApplication].SetDestination (gw);
